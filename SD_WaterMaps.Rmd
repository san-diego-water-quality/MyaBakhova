...
library(ggmap)
# san_diego_map <- get_googlemap(center = c(-117.2, 32.75),# "Mission Bay"
#                                maptype = "terrain",
#                                zoom = 12,
#                                size = c(640, 640),
#                                color = "color")
...
Citation: D. Kahle and H. Wickham. ggmap: Spatial Visualization with
...
At first I create a table with average values for every station for 
each year.
```{r YearlyValueMap, message=F, fig.width=10, fig.height=8}
library(dplyr)
yearlyResult <-  df %>% select(date_time, result, stationgroup,
     unit, analyte, targetlatitude, targetlongitude) %>%
    group_by(year=floor_date(date_time, "year"), stationgroup,
        unit, analyte, targetlatitude, targetlongitude) %>%
   summarize(yearlyMean = mean(result, na.rm = T)) 
dim(yearlyResult)
options(width=100)
head(yearlyResult)
```
...
ggmap(san_diego_map) +
      geom_point(data =
          ...
          color = stationgroup,
               size = logMeanPollution), shape =20)+
    ggtitle('Average Station Pollution Values for "Coliform, Total", 
        Measured as MPN/100 mL')
```